...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof userXP !== 'undefined' && (APP_DATA&&APP_DATA.currentUser)) {
...
function shareRoom() {
  ...
  if (navigator.share) {
    ...
    (navigator.clipboard?navigator.clipboard:{writeText:function(){return Promise.resolve()}}).writeText(url).then(() => showToast('تم نسخ الرابط 🔗','success')).catch(()=>showToast('رابط الغرفة: '+url,'info'));
  }
}
...
function processWithdraw() {
  const amt = parseInt((_el('withdrawAmt')).value || 0);
  ...
  (function(){var _e=document.querySelector('[style*="rgba(0,0,0,0.75)"]');return _e||{remove:function(){},classList:{add:function(){},remove:function(){},toggle:function(){}}};})().remove();
...
function processTransfer() {
  const to  = (_el('transferTo')).value.trim();
  const amt = parseInt((_el('transferAmt')).value || 0);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', e => {
    ...
    if (card && (_el('mainContent')).contains(card)) {
      const nameEl = card.querySelector('.speaker-name');
      const name = (nameEl && nameEl.textContent) ? nameEl.textContent.trim() : 'متحدث';
