...
function openSidebar() {
  ...
  (_el('sidebarOverlay')).classList.add('show');
  document.body.style.overflow = 'hidden';
}
function closeSidebar() {
  (_el('sidebar')).classList.remove('open');
  (_el('sidebarOverlay')).classList.remove('show');
  document.body.style.overflow = '';
}
function toggleMobileSearch() {
  var sb = document.querySelector('.search-bar'); if(sb) sb.classList.toggle('show');
}
  ...
  el.innerHTML = rooms.map(r => `
    <div class="room-card" onclick="navigate('room', ${r.id})">
      <div class="room-card-header">
        ...
        <span class="room-card-category">${(APP_DATA.categories.find(function(c){return c.id===r.category})||{}).name || r.category}</span>
      </div>
...
function addEmoji(e) {
  ...
  (_el('emojiPickerBox')).classList.add('hidden');
}
...
function toggleEmoji(inputId) {
  ...
  (_el('emojiPickerBox')).classList.toggle('hidden');
}
...
function showGiftsQuick() {
  ...
  document.querySelectorAll('.chat-tab').forEach(t => { var oc=t.getAttribute('onclick'); if(oc && oc.includes('gifts')) t.classList.add('active'); else t.classList.remove('active'); });
}
...
function saveProfile() {
  const name   = (_el('ep_name')).value.trim();
  const handle = (_el('ep_handle')).value.trim();
  const bio    = (_el('ep_bio')).value.trim();
...
function createRoom() {
  const name = (_el('roomName')).value.trim();
  if (!name) { showToast('ادخل اسم الغرفة', 'error'); return; }

  const selectedType = (document.querySelector('.room-type-item.selected .type-name')||{textContent:'',value:'',remove:function(){}}).textContent || 'موسيقى';
  const catMap = { 'موسيقى':'music','تقنية':'tech','كوميديا':'comedy','رياضة':'sports','ألعاب':'gaming','قرآن':'quran' };
  const privacy = (document.querySelector('input[name="roomPrivacy"]:checked')||{textContent:'',value:'',remove:function(){}}).value || 'public';

  const newRoom = {
    id: APP_DATA.rooms.length + 100,
    title: name,
    desc: (_el('roomDesc')).value.trim() || 'غرفة جديدة مباشرة',
...
function handleLogin() {
  const phone = (_el('loginPhone')).value.trim();
  const pass  = (_el('loginPass')).value.trim();
  if (!phone && !pass) {
...
function handleRegister() {
  const name  = (_el('regName')).value.trim();
  const phone = (_el('regPhone')).value.trim();
  const pass  = (_el('regPass')).value.trim();
...
function completeGoogleLogin(name, email) {
  var el = document.querySelector('[style*="rgba(0,0,0,0.8)"]'); if(el) el.remove();
  const uid = generateUserId();
  APP_DATA.currentUser.id = uid;
  APP_DATA.currentUser.name = name;
  APP_DATA.currentUser.initials = name.split(' ').map(w=>w[0]).join('').slice(0,2);
